...
> **Amended 2026-09-14 (PR #124).** The "in every other case matching is positional" half of the paragraph above no longer holds for a class that *declares* an element identity. Field evidence (consolidator-server #400/#399) showed that a list whose class declares a key but whose data repeats or omits a label, and therefore fell back to positional segments, produced deltas that were silently re-read as label segments against a base whose data did honour the key — wrong elements edited, no error. `diff` now decides addressing from the schema: a declaring class whose data is not keyed-shaped on both sides gets **no per-element segments**, only one whole-slot `Update`. The anti-collapse half stands unchanged: two elements sharing a label are never merged, the engine simply declines to address them individually. Classes that declare nothing keep positional segments exactly as the Non-goal says. Full reasoning and the tests that flipped: `docs/superpowers/plans/2026-09-14-keyed-list-addressing-not-recorded-in-delta.md`. The single-list resolvers (`patch`, `navigate_path`, `list_path_segments`) are still data-shaped; see issue #126.
